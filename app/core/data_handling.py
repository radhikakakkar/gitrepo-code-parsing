...
import shutil
from model import Repositories
import re
from db import db, repositories_data, files_data, functions_data
from .tree_sitter_parsing import parse_files_for_function_data
...
def get_file_names(repo_dir, repo_name, repo_object_id):
    ...
    return {"status":"done", "message": "file names saved in files_data collection", "data": relevant_file_paths}
        
#extract function names for each file and save function_names field in files_data, save function names as seperate docs in functions_data
def get_function_names(file_paths):
    for file_path in file_paths:
        print(file_path)
        file_function_data = parse_files_for_function_data(file_path) # giving all the functions names and data in a list of objects
        ...
        if len(file_function_data) != 0:
            ...
            if not update_funtions_data.acknowledged:
                return {"status": "error", "message": "error saving function data for {file_path}"}
        # else:
...
def extract_meta_data(repo_name):
    ...
    if file_names_result["status"] == "done" and functions_data_result["status"] == "done":
