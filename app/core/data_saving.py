...
from .tree_sitter_parsing import parse_files_for_function_data, parse_files_for_class_data
...
def get_class_names(file_paths):
    for file_path in file_paths:
        ...
        print(f" type for data for {file_path} {type(file_class_data)}") # should be list 
        if len(file_class_data) != 0:
            ...
            if not update_class_data:
                return {"status": "error", "message": "error saving class data for {file_path}"}
